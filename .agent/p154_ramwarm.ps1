...
$usageName = 'nin' + $F + 'er-expert-usage.txt'
$log       = Join-Path $root '.agent\p154_ramwarm.log'
$testOut   = Join-Path $root '.agent\p154_test_stdout.log'
$testErr   = Join-Path $root '.agent\p154_test_stderr.log'
$usageFile = Join-Path $root $usageName
$envMoe     = 'NIN' + $F + 'ER_MOE_CPU_LAYERS'
$envM3      = 'NIN' + $F + 'ER_P13_M3'
$envTim     = 'NIN' + $F + 'ER_P13_TIMING'
$envMaxTP   = 'NIN' + $F + 'ER_P13_M3_MAXTP'
$envPin     = 'NIN' + $F + 'ER_EXPERT_PINNED_SLOTS'
$envRamWarm = 'NIN' + $F + 'ER_EXPERT_RAM_WARM'
$envUsage   = 'NIN' + $F + 'ER_EXPERT_USAGE_FILE'
# Prefill residency sizing (bounded for the 44 GB free RAM):
#   pinned 2000 -> ranks [7191, 9191) into cudaHostAlloc pinned RAM (~5.3 GB)
#   ram-warm 10000 -> top 10000 mmap pages faulted into page cache (~26 GB)
#   GPU tier stays DYNAMIC (7191, top usage rank) — the fastest, zero-DMA tier.
$PIN_SLOTS  = '2000'
$RAM_WARM   = '10000'
...
W "P154 RAM-WARM MEASURE START (CPU-OFF, P13, MAXTP=512, PINNED_SLOTS=" + $PIN_SLOTS + ", RAM_WARM=" + $RAM_WARM + ", 45-min hard kill)"

# ---- 0. Record live serve oracle ----
...
# ---- 1. Verify exe + model + ngram + usage file BEFORE touching serve ----
...
if (-not (Test-Path -LiteralPath $usageFile))  { W "MISSING usage: $usageFile (run the M3 baseline once first)"; $ok = $false }
...
if (-not $killed -or $free -ge 30000) {
  # ---- 4. Seeded M3 T=1 decode + 512 prefill, CPU OFF ----
  ...
  [Environment]::SetEnvironmentVariable($envPin, $PIN_SLOTS, 'Process')
  [Environment]::SetEnvironmentVariable($envRamWarm, $RAM_WARM, 'Process')
  [Environment]::SetEnvironmentVariable($envUsage, $usageFile, 'Process')
  W ("M3 SEEDED START (PINNED_SLOTS=" + $PIN_SLOTS + ", RAM_WARM=" + $RAM_WARM + ", MAXTP=512, 45-min hard kill)")
  # Single pre-quoted arg string (spaced project-root paths).
...
# ---- 5. Wait for the test's VRAM + RAM to free before restarting the 27B serve ----
if ($killed) {
  ...
  W ("post-test VRAM free MiB=" + $free2 + " RAM free GB=" + $freeRamGb)
}
...
W "================================================"
W ("  P154 RAM-WARM MEASURE DONE (test exit=$code; timeout=$timedOut; serve restarted=$($back.Count -gt 0))")
