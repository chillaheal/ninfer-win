...
LoadedModelData::LoadedModelData(BindingPlan plan, artifact::MaterializedArtifact materialized)
    ...
    if (plan.features.vision) {
        ...
        const bool host = plan.features.vision_cpu;
        vision.common = qwen3_6::materialize_vision_common(
            backing, plan.vision_backbone, plan.vision_merger_input, plan.vision_merger_norm, host);
        vision.merger_fc2 =
            host ? artifact::materialized_host_weight(backing, plan.vision_merger_fc2,
                                                       NumericFormat::W8G32_F16S, 5120, 4608)
                 : artifact::materialized_weight(backing, plan.vision_merger_fc2,
                                                 NumericFormat::W8G32_F16S, 5120, 4608);
        vision.merger_fc2_bias =
            host ? artifact::materialized_host_tensor(backing, plan.vision_merger_fc2_bias,
                                                       NumericFormat::BF16, {5120})
                 : artifact::materialized_tensor(backing, plan.vision_merger_fc2_bias,
                                                 NumericFormat::BF16, {5120});
    }
}
