...
void propose_dflash2_batch(DFlashBatchContext& state, qwen3_6::DFlashDecodeState& frame, int batch,
    ...
    if constexpr (V::DFlashConfig::coherent_selector) {
        ...
        if (state.execution.ngram) {
            constexpr float kNgramBoost = 1000.0F;
            ops::ngram_candidate_inject(
                static_cast<std::int32_t*>(candidates.data), static_cast<float*>(scores.data),
                static_cast<const std::int32_t*>(frame.ngram_tokens.data),
                static_cast<const std::int32_t*>(frame.ngram_counts.data), k, batch,
                /*ngram_concurrency=*/8, kNgramBoost, stream);
        }
