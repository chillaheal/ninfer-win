...
namespace {
// Host-side self-speculation n-gram self-match: find the most-recent earlier occurrence of the
// trailing `order`-gram of `ledger` and emit the up-to-`K` continuation tokens that followed it,
// written draft-major into out_tokens[d * 8 + row]. Returns the number m of valid continuation
// tokens (0 when the n-gram has no earlier occurrence). This runs on the host, outside the CUDA
// graph, in the round driver; only the resulting merge-kernel launch is capturable.
std::int32_t ngram_self_spec_match(const TokenId* ledger, std::size_t ledger_len,
                                 std::int32_t* out_tokens, int row, int K, int order,
                                 int search_window) {
    const std::size_t n = static_cast<std::size_t>(order);
    if (K < 1 || order < 1 || ledger_len < n + 1) {
        return 0;
    }
    const TokenId* tail    = ledger + (ledger_len - n);
    const std::size_t start = ledger_len - n - 1;
    std::size_t floor_    = 0;
    if (search_window > 0 && start >= static_cast<std::size_t>(search_window)) {
        floor_ = start - static_cast<std::size_t>(search_window);
    }
    for (std::size_t j = start; ; --j) {
        bool match = true;
        for (std::size_t k = 0; k < n; ++k) {
            if (ledger[j + k] != tail[k]) {
                match = false;
                break;
            }
        }
        if (match) {
            const std::size_t followers = ledger_len - (j + n);
            const std::size_t m =
                static_cast<std::size_t>(K) < followers ? static_cast<std::size_t>(K) : followers;
            for (std::size_t d = 0; d < m; ++d) {
                out_tokens[d * 8 + static_cast<std::size_t>(row)] =
                static_cast<std::int32_t>(ledger[j + n + d]);
            }
...
ProgramImplCore::decode_dflash_batch(std::span<const std::uint32_t> lanes,
    ...
    try {
        ...
        for (std::size_t row = 0; row < lanes.size(); ++row) {
            ...
            if (ngram) {
                dflash_host_ingress->ngram_counts[row] = ngram_self_spec_match(
                    sequence.ledger.data(), sequence.ledger.size(),
                    dflash_host_ingress->ngram_tokens.data(), static_cast<int>(row),
                    static_cast<int>(extent), /*order=*/8, /*search_window=*/4096);
            }
            dflash_host_ingress->execution_frontiers[row] =
