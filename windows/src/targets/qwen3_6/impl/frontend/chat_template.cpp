...
constexpr Sha256Digest kCommunityTemplateDigest{
...
CompiledChatTemplate CompiledChatTemplate::resolve(std::string_view source) {
    ...
    if (digest == kThinkingToggleTemplateDigest) {
        return CompiledChatTemplate(ChatTemplateSemantics::ThinkingToggle, hex);
    }
...
PromptCapabilities CompiledChatTemplate::capabilities() const {
    PromptCapabilities result;
    result.enable_thinking     = true;
    result.template_sha256_hex = digest_hex_;
