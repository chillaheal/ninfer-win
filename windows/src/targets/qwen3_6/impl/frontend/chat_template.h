...
public:
    ...
    [[nodiscard]] PromptCapabilities capabilities() const;
    [[nodiscard]] RenderedChat render(const std::vector<ChatMessage>& messages,
                                      ChatRenderOptions options = {}) const;

private:
    CompiledChatTemplate(ChatTemplateSemantics semantics, std::string digest_hex) noexcept
        : semantics_(semantics), digest_hex_(std::move(digest_hex)) {}
