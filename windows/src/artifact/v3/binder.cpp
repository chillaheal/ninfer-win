...
float HostValues::scalar_f32() const {
    if (format != QType::FP32_CTRL || elements != 1 || data.size() != 4) {
        throw ArtifactError("auxiliary requires one represented FP32_CTRL value");
    }
    return std::bit_cast<float>(read_u32_le(data.data()));
}

std::vector<std::int32_t> HostValues::integers() const {
    if (format != QType::I32_CTRL || data.size() != checked_mul(elements, 4, "integer values")) {
        throw ArtifactError("semantic table requires I32_CTRL values");
    }
...
HostValues Binder::values(const Binding& binding, std::optional<QType> format) {
    ...
    if (out.format == QType::BF16_CTRL) {
        word_bytes = 2;
    } else if (out.format == QType::FP32_CTRL || out.format == QType::I32_CTRL) {
        word_bytes = 4;
    } else {
