...
public:
    explicit MappedFile(const std::filesystem::path& path) {
#ifdef _WIN32
        handle_ = ::CreateFileW(path.wstring().c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr,
                                OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
        if (handle_ == INVALID_HANDLE_VALUE) {
            // Win32 error codes — system_category maps them to real messages on MSVC.
            throw std::system_error(static_cast<int>(::GetLastError()), std::system_category(),
                                    "open " + path.string());
        }

        LARGE_INTEGER file_size {};
        if (::GetFileSizeEx(handle_, &file_size) == 0 || file_size.QuadPart < 0 ||
        ...
        if (size != 0) {
            mapping_ = ::CreateFileMappingW(handle_, nullptr, PAGE_READONLY,
            ...
            mapping = ::MapViewOfFile(mapping_, FILE_MAP_READ, 0, 0, size);
            if (mapping == nullptr) {
                const DWORD error = ::GetLastError();
...
#ifdef _WIN32
        // Windows has no O_DIRECT; the mapped view already backs the whole file, so direct
        // reads are served from it. The 4096-byte alignment contract is preserved for callers.
        // A read that starts in-file but extends past EOF is short-read to the in-file
        // portion, mirroring the POSIX path where pread returns a short count at EOF: the
        // materializer aligns the final read of a span up to 4096 bytes, which over-reads
        // when the file ends mid-alignment-unit. A start past EOF is still a hard error.
        if (absolute_offset > static_cast<std::uint64_t>(size_)) {
            throw ArtifactError("direct artifact read exceeds file bounds");
        }
        const std::size_t bytes =
            std::min(destination.size(), size_ - static_cast<std::size_t>(absolute_offset));
        std::memcpy(destination.data(), data_ + absolute_offset, bytes);
        return bytes;
#else
