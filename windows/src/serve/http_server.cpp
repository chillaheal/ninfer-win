...
const char* effort_name(ninfer::ReasoningEffort effort) {
    switch (effort) {
...
std::string HttpServer::health_payload(const std::string& model_id,
    ...
    template_json["sha256"] = capabilities.template_sha256_hex;
    const bool effort = capabilities.reasoning_effort.low ||
                        capabilities.reasoning_effort.medium ||
                        capabilities.reasoning_effort.xhigh;
    template_json["semantics"]       = effort ? "reasoning_effort" : "thinking_toggle";
    template_json["enable_thinking"] = capabilities.enable_thinking;
    if (effort) {
        nlohmann::json supported = nlohmann::json::array();
        if (capabilities.reasoning_effort.low) { supported.push_back("low"); }
        if (capabilities.reasoning_effort.medium) { supported.push_back("medium"); }
        if (capabilities.reasoning_effort.xhigh) { supported.push_back("xhigh"); }
        nlohmann::json effort_json;
        if (capabilities.reasoning_effort.default_effort) {
            effort_json["default"] = effort_name(*capabilities.reasoning_effort.default_effort);
        }
        effort_json["supported"] = std::move(supported);
    ...
    if (!options.system_prompt_file.empty()) {
        ...
        system_prompt["active"] = !options.system_prompt_default.empty();
        system_prompt["bytes"]  = options.system_prompt_default.size();
        body["system_prompt"]   = std::move(system_prompt);
    }
    return body.dump();
}
...
void HttpServer::register_routes() {
    ...
    server_.Get("/health", [this](const httplib::Request&, httplib::Response& res) {
        // Before the service is attached (model still loading) keep the legacy minimal payload.
        if (service_ == nullptr) {
            res.set_content(nlohmann::json{{"status", "ok"}}.dump(), "application/json");
            return;
        }
        res.set_content(health_payload(public_model_id_, service_->prompt_capabilities(),
                                       options_),
                        "application/json");
    });
