...
ServeOptions parse_serve_options(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "--host") {
            ...
            if (options.system_prompt_file.empty()) {
                throw std::invalid_argument("--system-prompt-file must not be empty");
            }
            // Read once at parse (fail fast, before model loading). Absent or empty after
            // trimming trailing whitespace is a valid no-default configuration; present but
            // unreadable is a hard error.
            const std::filesystem::path prompt_path(options.system_prompt_file);
            if (std::filesystem::exists(prompt_path)) {
                std::ifstream prompt_file(prompt_path, std::ios::binary);
                if (!prompt_file) {
                    throw std::invalid_argument("cannot read --system-prompt-file: " +
                                                options.system_prompt_file);
                }
                std::ostringstream prompt_text;
                prompt_text << prompt_file.rdbuf();
                if (prompt_file.bad()) {
                    throw std::invalid_argument("cannot read --system-prompt-file: " +
                                                options.system_prompt_file);
                }
                std::string content = prompt_text.str();
                const std::size_t last = content.find_last_not_of(" \t\r\n");
                content.erase(last == std::string::npos ? 0 : last + 1);
                options.system_prompt_default = std::move(content);
            }
    ...
    if (options.kv_capacity.mode == KvCapacityMode::Explicit) {
        // The engine's sequence capacity curve caps the pool at
        // max_context x max_concurrency pages; a wider pool dies deep in the
        // target with an opaque curve error, so reject it here with an
        // actionable message.
        const std::uint64_t window_tokens =
            static_cast<std::uint64_t>(options.max_context) * options.max_concurrency;
        if (options.kv_capacity.explicit_tokens > window_tokens) {
            throw std::invalid_argument(
                "--kv-capacity must not exceed --max-context x --max-concurrency: the "
                "engine's sequence capacity curve rejects a KV pool wider than the "
                "context window; raise --max-context to at least --kv-capacity");
        }
    }
