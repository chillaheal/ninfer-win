...
public:
    ...
    // The /health body: status + model id + loaded chat-template identity/capabilities +
    // system-prompt source. Pure (no I/O) so it is unit-tested directly. `system_prompt` is
    // omitted when the flag was absent.
    [[nodiscard]] static std::string health_payload(const std::string& model_id,
