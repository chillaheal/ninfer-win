...
std::string format_request_start(const RequestLogContext& context) {
    // Compact console line: what is happening now. Full-precision request detail
    // (sampling, thinking, media preparation) lives in the JSONL start record.
    std::ostringstream out;
    out << "[req " << context.id << "] start " << context.protocol << ' '
        << (context.stream ? "stream" : "non-stream") << " msgs=" << context.message_count
        << " tools=" << context.tool_count << " \xE2\x86\x92 submitted";
    return out.str();
}
...
std::string format_request_done(const RequestLogContext& context,
    ...
    // Compact console line: outcome plus prefill/decode rates. Full-precision timings,
    // cache-reuse, host exposure, speculative and thinking accounting live in the JSONL
    // done record.
    std::ostringstream out;
...
std::string format_throughput(const ThroughputReport& report) {
    // Compact console heartbeat: current rates and active request state. Batch,
    // host-work and maintenance deltas are only reported in the JSONL record.
    ...
    const double decode_rate =
        report.interval_seconds > 0.0
            ...
            : 0.0;
    std::ostringstream out;
    out << "throughput prefill=" << std::fixed << std::setprecision(1) << prefill_rate << "tok/s"
        << " decode=" << decode_rate << "tok/s running=" << report.current.running_requests
