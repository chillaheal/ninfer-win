...
void ngram_candidate_inject(std::int32_t* candidate_ids, float* unary,
                            ...
                            cudaStream_t stream) {
    if (K < 1 || K > 15) throw std::invalid_argument("ngram_candidate_inject: K must be in [1,15]");
    if (B < 1 || B > 8) throw std::invalid_argument("ngram_candidate_inject: B must be in [1,8]");
    detail::ngram_candidate_inject_launch(ngram_tokens, ngram_counts, candidate_ids, unary, K, B,
                                         ngram_concurrency, boost, stream);
}
