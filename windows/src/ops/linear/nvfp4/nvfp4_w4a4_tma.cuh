...
__launch_bounds__(Schedule::kThreads, Schedule::kMinBlocksPerSm) void nvfp4_w4a4_tma_kernel(
    ...
    // host wrapper (C2719 — it rejects even the host-side declaration), so on Windows the
    // descriptor block is staged in device memory and passed as a pointer. The TMA unit reads
    // it from memory during execution, exactly like upstream's by-value parameter buffer.
