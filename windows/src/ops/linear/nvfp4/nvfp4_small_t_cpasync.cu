...
#include "ops/linear/nvfp4/nvfp4_small_t_cpasync.cuh"
...
// Pipeline depth for the cp.async staging. 4 stages keeps ~3 phases of DRAM traffic in
// flight; the shared-memory cost is Stages * kRowsPerCta * kPhaseCodeBytes bytes.
constexpr int kNvfp4SmallTCpAsyncStages = 4;
...
void launch_exact(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    ...
    nvfp4_small_t_cpasync_kernel<Geometry, ActiveTokens, Schedule, kNvfp4SmallTCpAsyncStages>
...
void launch_nvfp4_small_t_cpasync(const Tensor& x, const Weight& weight, Tensor& out,
                                  cudaStream_t stream) {
