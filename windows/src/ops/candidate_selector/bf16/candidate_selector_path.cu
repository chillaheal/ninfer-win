...
// Self-speculation candidate injection: for each (b, i) column where lane b has a context ngram
// match (i < ngram_counts[b]), overwrite the weakest candidate slot (rank 15) with the ngram
// continuation token for that step and boost its unary score so the selector considers it.
// ngram_tokens is [kDFlashDecodeMaximumDrafts][kMaximumConcurrency] (draft-major); column b*K+i
// is the (b, i) column and its weakest slot is rank kCandidates-1.
__global__ void ngram_candidate_inject_kernel(const std::int32_t* ngram_tokens,
                                       const std::int32_t* ngram_counts, std::int32_t* candidate_ids,
                                              float* unary, int steps, int batch,
                                              int ngram_concurrency, float boost) {
    const int column = blockIdx.x * blockDim.x + threadIdx.x;
    if (column >= batch * steps) return;
    const int b = column / steps;
    const int i = column % steps;
    if (i >= ngram_counts[b]) return;
    const int tok = ngram_tokens[i * ngram_concurrency + b];
    candidate_ids[column * kCandidates + (kCandidates - 1)] = tok;
    unary[column * kCandidates + (kCandidates - 1)] = boost;
}
...
void ngram_candidate_inject_launch(const std::int32_t* ngram_tokens, const std::int32_t* ngram_counts,
                                   std::int32_t* candidate_ids, float* unary, int steps, int batch,
                                   int ngram_concurrency, float boost, cudaStream_t stream) {
    const int columns = batch * steps;
    const int block    = 256;
    const int grid     = (columns + block - 1) / block;
    ngram_candidate_inject_kernel<<<grid, block, 0, stream>>>(ngram_tokens, ngram_counts,
                                                                 candidate_ids, unary, steps,
                                                                     batch, ngram_concurrency,
                                                                     boost);
    CUDA_CHECK(cudaGetLastError());
}
} // namespace ninfer::ops::detail
