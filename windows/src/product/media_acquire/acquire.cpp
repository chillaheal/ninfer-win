...
std::vector<std::uint8_t> read_path(const Source& source, const Policy& policy) {
    ...
    if (!policy.media_root.empty()) {
        ...
        // path("..").native() is the platform-native spelling (".." on POSIX, L".." on Windows),
        // matching the native string type of relative.
        const auto outside_root = relative.native().starts_with(std::filesystem::path("..").native());
        if (ec || relative.empty() || outside_root) {
