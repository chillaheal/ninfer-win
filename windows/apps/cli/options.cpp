...
std::string usage_text(const char* argv0) {
    return std::string("usage: ") + argv0 +
           " <model.ninfer> (--prompt <text>|--messages <messages.json>|--probe)\n"
           ...
           "--probe prints key=value lines (model, VRAM, and the largest KV capacity that fits\n"
           "current free VRAM after a 3% margin) to stdout without loading weights; the GUI\n"
           "Auto button uses it.\n"
...
Options parse_options(int argc, char** argv) {
    ...
    if (!options.probe_requested && has_prompt == has_messages) {
        throw std::invalid_argument("pass exactly one of --prompt or --messages");
    }
    if (options.prefill_chunk % 128 != 0) {
        throw std::invalid_argument("--prefill-chunk must be a multiple of 128");
    }
    // Probe mode ignores max_context/kv_capacity (it derives the fit from live VRAM), so the
    // pairing invariant only applies to actual generation runs.
    if (!options.probe_requested && options.kv_capacity.mode == KvCapacityMode::Explicit &&
