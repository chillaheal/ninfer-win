#ifdef _WIN32
// Port shim: the MSVC CRT encodes narrow argv with the ANSI code page, which corrupts
// non-ASCII text; the apps re-encode the wide argv as UTF-8 (see common/utf8_argv.h).
#include "common/utf8_argv.h"
#endif
...
int run_cli(int argc, char** argv) {
    try {
        ...
        // Probe mode: report the largest KV capacity that fits current free VRAM, then exit.
        // No prompt/messages required; no weights are materialized and no model is built, so
        // this completes in seconds. Key=value lines go to stdout (parsed by the GUI Auto
        // button); a timing note goes to stderr so it does not pollute the machine-readable
        // output.
        if (cli.probe_requested) {
            ...
            std::cout << "ninfer-probe model=" << probe.model_id << '\n'
                      << "vram_total_bytes=" << probe.vram_total_bytes << '\n'
                      << "weights_bytes=" << probe.weights_bytes << '\n'
                      << "vram_free_after_weights_bytes=" << probe.vram_free_after_weights_bytes
                      << '\n'
                      << "kv_headroom_percent=" << probe.kv_headroom_percent << '\n'
                      << "kv_fit_tokens=" << probe.kv_fit_tokens << '\n';
            std::cerr << "probe completed in "
                      << format_seconds(
...
#else
// The MSVC CRT delivers narrow argv through the ANSI code page (even with /utf-8), which
// corrupts non-ASCII arguments; wmain receives the original UTF-16 command line instead.
int wmain(int argc, wchar_t** argv) { return ninfer::port::run_wmain(argc, argv, &run_cli); }
#endif
