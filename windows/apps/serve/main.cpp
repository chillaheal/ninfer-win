...
// non-ASCII text (e.g. model paths); the apps re-encode the wide argv as UTF-8.
...
int run_serve(int argc, char** argv) {
    ...
    if (!options.system_prompt_file.empty()) {
        ninfer::serve::write_console_log(
            ...
            options.system_prompt_default.empty()
                ? "system prompt file " + options.system_prompt_file +
                      ": absent or empty — no server default"
                : "system prompt file " + options.system_prompt_file + ": " +
                      std::to_string(options.system_prompt_default.size()) + " bytes active");
    }
...
#ifndef _WIN32
int main(int argc, char** argv) { return run_serve(argc, argv); }
#else
...
int wmain(int argc, wchar_t** argv) { return ninfer::port::run_wmain(argc, argv, &run_serve); }
#endif
