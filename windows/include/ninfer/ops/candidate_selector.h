...
// Self-speculation (self-speculation) ngram candidate injection for the DFlash2 selector. For each
// (b, i) column where lane b has a context ngram match (i < ngram_counts[b]), overwrite the weakest
// candidate slot (rank kCandidates-1) with the ngram continuation token for step i and set its
// unary score to `boost`, so the selector considers the ngram continuation alongside dflash2's 16.
// Pure kernel launch (a capturable CUDA-graph node); a no-op per column when ngram_counts[b]==0.
// candidate_ids/unary are the rank-fastest [16,K,B] bases candidate_selector_path consumes;
// ngram_tokens is [kDFlashDecodeMaximumDrafts][kMaximumConcurrency] (draft-major), so step i,
// lane b is ngram_tokens[i * ngram_concurrency + b].
void ngram_candidate_inject(std::int32_t* candidate_ids, float* unary,
                            const std::int32_t* ngram_tokens, const std::int32_t* ngram_counts,
                            int K, int B, int ngram_concurrency, float boost,
                            cudaStream_t stream);
