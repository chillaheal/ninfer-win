...
Exactly one of `--prompt`, `--messages`, and `--probe` is required. The CLI normally omits
`--kv-capacity`, so the shared Main Text KV pool follows the example's 32,768-token `--max-context`.
...
`--probe` plans a load without materializing weights and reports the largest KV window that fits
the current free GPU memory. It performs no generation and finishes in seconds:

```bash
./build/apps/ninfer models/qwen3_8_27b_nvfp4.ninfer --probe
```

The report is key=value lines on stdout; a timing note goes to stderr:

```text
ninfer-probe model=qwen3.8-27b
vram_total_bytes=34190458880
weights_bytes=20375621632
vram_free_after_weights_bytes=12120797184
kv_headroom_percent=3
kv_fit_tokens=171520
```

`kv_fit_tokens` is the largest context window whose KV cache and capacity-scaled attention
workspace fit in the free memory after weights, leaving a 3% headroom. The GUI **Auto** button
runs this probe and fills the Max-context field with `kv_fit_tokens`.
