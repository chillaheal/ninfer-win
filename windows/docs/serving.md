...
`medium`, and `xhigh`; `high` rounds up to `xhigh`. Omitting effort uses that template's
declared default. An explicit effort
...
select the corresponding template effort when available, and `high` rounds up to `xhigh`. The
remaining OpenAI protocol values `minimal` and `max` are parsed but rejected for the registered
templates.
...
| `reasoning.effort` | `none` disables thinking; `low`, `medium`, or `xhigh` selects an effort exposed by the loaded chat template; `high` rounds up to `xhigh`; `minimal` and `max` return `reasoning_effort_not_supported` for the registered templates |
...
endpoints; the registered effort-capable template exposes `low`, `medium`, and `xhigh` (`high`
rounds up to `xhigh`; `max` is rejected). Combining
...
| `--system-prompt-file FILE` | server-side system prompt for requests that omit or send an empty `system` | unset |

`--system-prompt-file FILE` sets a server-side system prompt, read once at startup as raw
UTF-8 bytes. Per request, the leading system instruction resolves as: a non-empty request
`system` wins (the file is not appended); an absent or empty request `system` gets the file
content as a leading system turn at index 0; mid-conversation system messages are not
considered. A missing or empty file means no default (behavior unchanged); an unreadable
file aborts startup with a clear message. The injected text counts against `max_context`
like any prompt text and is included in `count_tokens`. `/health` reports the loaded chat
template under `chat_template` — `sha256` (digest of the template source), `semantics`
(`reasoning_effort` or `thinking_toggle`), `enable_thinking`, and, for reasoning-effort
templates, a `reasoning_effort` block with `default` and `supported` — and, only when the
flag is set, a `system_prompt` block with `file`, `active`, and `bytes`.
