...
ninfer::PromptInput translate_with_default(const GenerationRequest& req,
                                           const std::string& server_default) {
    const ServeOptions server = default_server();
    return to_prompt_input(req, resolve_prompt_semantics(req, server, effort_capabilities()),
                           fake_media, server_default);
}
...
int test_reasoning_effort() {
    ...
    const ninfer::PromptInput high_prompt = translate(high_request);
    failures += check(high_prompt.options.enable_thinking &&
                          high_prompt.options.reasoning_effort == ninfer::ReasoningEffort::XHigh,
                      "Anthropic high effort did not round up to the closest supported level");
...
int test_system_prompt_default_injection() {
    int failures       = 0;
    const std::string server_default = "Server default system prompt.";
    const Json base = {{"model", "m"},
                       {"max_tokens", 64},
                       {"messages",
                        Json::array({Json{{"role", "user"}, {"content", "hello"}}})}};
    const GenerationRequest no_system = parse_messages_request(base, default_limits());

    // Row 1: client system non-empty + default set => client only, never concatenated.
    const Json with_client = {{"model", "m"},
                              {"max_tokens", 64},
                              {"system", "client says hi"},
                              {"messages",
                               Json::array({Json{{"role", "user"}, {"content", "hello"}}})}};
    const GenerationRequest client_req = parse_messages_request(with_client, default_limits());
    const ninfer::PromptInput client_wins = translate_with_default(client_req, server_default);
    failures += check(client_wins.messages.size() == 2, "client system: turn count unchanged");
    failures += check(client_wins.messages[0].role == ninfer::ChatRole::System &&
                          joined_text(client_wins.messages[0]) == "client says hi",
                      "client system: client text only, no server default");

    // Row 2: no client system + default set => server turn prepended at index 0.
    const ninfer::PromptInput prepended = translate_with_default(no_system, server_default);
    failures += check(prepended.messages.size() == 2, "no client system: server turn added");
    failures += check(prepended.messages[0].role == ninfer::ChatRole::System &&
                          joined_text(prepended.messages[0]) == server_default,
                      "no client system: server default at index 0");
    failures += check(prepended.messages[1].role == ninfer::ChatRole::User &&
                          joined_text(prepended.messages[1]) == "hello",
                      "no client system: user turn follows");

    // Row 3: empty client system + default set => the empty turn is replaced in place
    // (anthropic_schema.cpp keeps the empty-text System turn; shape is preserved).
    const Json empty_client = {{"model", "m"},
                               {"max_tokens", 64},
                               {"system", ""},
                               {"messages",
                                Json::array({Json{{"role", "user"}, {"content", "hello"}}})}};
    const GenerationRequest empty_req = parse_messages_request(empty_client, default_limits());
    const ninfer::PromptInput replaced = translate_with_default(empty_req, server_default);
    failures += check(replaced.messages.size() == 2, "empty client system: turn count unchanged");
    failures += check(
        (replaced.messages[0].role == ninfer::ChatRole::System ||
         replaced.messages[0].role == ninfer::ChatRole::Developer) &&
            joined_text(replaced.messages[0]) == server_default,
        "empty client system: replaced with server default");

    // Row 4: no default => unchanged behavior, no injection.
    const ninfer::PromptInput untouched = translate_with_default(no_system, "");
    failures += check(untouched.messages.size() == 1, "no default: no injection");
    failures += check(untouched.messages[0].role == ninfer::ChatRole::User,
                      "no default: user turn only");

    // count_prompt_tokens honesty: GenerationService passes the identical 4th argument at
    // both to_prompt_input call sites (generation_service.cpp), so the counted input is
    // exactly the prepended input asserted above.
    failures += check(prepended.messages.size() == 2 &&
                          joined_text(prepended.messages[0]) == server_default,
                      "count path: same to_prompt_input input as generation (server turn counted)");
    return failures;
}
...
int main() {
    ...
    failures += test_system_prompt_default_injection();
