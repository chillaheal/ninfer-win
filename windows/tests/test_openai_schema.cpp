...
int test_reasoning_effort() {
    ...
    const auto high_semantics            =
        resolve_prompt_semantics(high_request, default_server(), effort_capabilities());
    failures += check(high_semantics.enable_thinking &&
                          high_semantics.reasoning_effort == ninfer::ReasoningEffort::XHigh,
                      "high effort did not round up to the closest supported level");
    ...
    failures += check(api_code([&] {
                      ...
                      "high effort was accepted by a template without reasoning effort support");
    failures += check(api_code([&] {
