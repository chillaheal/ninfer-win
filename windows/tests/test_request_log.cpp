...
int main() {
    ...
    // The human-readable console line is deliberately compact: status plus prefill/decode
    // rates. Full-precision detail (sampling, thinking, reuse, host exposure) is asserted
    // against the JSONL records above.
    failures +=
        check(format_request_start(context).find("start openai_chat_completions non-stream") !=
                  std::string::npos &&
                  format_request_start(context).find("msgs=2") != std::string::npos,
              "human request start line lost protocol/stream/message shape");
    failures += check(format_request_done(context, outcome).find("finish=output_limit") !=
                          std::string::npos &&
                      format_request_done(context, outcome).find("prompt=401") !=
                          std::string::npos &&
                      format_request_done(context, outcome).find("gen=1024") != std::string::npos,
                      "human request done line lost outcome summary");
    failures +=
        check(format_request_done(context, outcome).find("prefill=1278.9tok/s") !=
                  std::string::npos &&
                  format_request_done(context, outcome).find("decode=191.4tok/s") !=
                      std::string::npos &&
                  format_request_done(context, outcome).find("ttft=358ms") != std::string::npos &&
                  format_request_done(context, outcome).find("wall=5.70s") != std::string::npos,
              "human request done line lost prefill/decode rates or timings");
    ...
    failures += check(human_throughput.find("prefill=50.0tok/s") != std::string::npos &&
                          ...
                          human_throughput.find("running=2") != std::string::npos &&
                          human_throughput.find("prefilling=1") != std::string::npos &&
                          human_throughput.find("decode_ready=1") != std::string::npos,
                      "human throughput heartbeat lost rates or request state");
    ...
    const std::filesystem::path log_path =
        ...
        ("ninfer-request-log-test-" + std::to_string(current_process_id()) +
         ".jsonl");
